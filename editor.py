...
class Editor(DirectObject):
    ...
    def __init__(self, map_manager):
        ...
        # добавьте новое свойство - значение текущего режима
        # ...
        ...
        self.accept('mouse3', self.addBlock)
        # регистрируем на нажатие правой кнопки мыши
        # событие удаления блока
        self.accept('mouse1', self.delBlock)

    # Добавьте метод установки режима редактирования
    def setEditMode(self, mode):
        # обновите значение текущего режима
        self.edit_mode = mode
        # если установлен «Режим редактора»
        if self.edit_mode:
            # сбрасываем свойства выделенного блока
            self.resetSelectedBlock()
            # запускаем задачу проверки выделения блоков
            taskMgr.doMethodLater(0.02, self.testBlocksSelection, "test_block-task")
        # иначе
        else:
